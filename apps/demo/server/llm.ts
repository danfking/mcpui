...
async function* streamResponseApi(
    ...
    // Truncate prior assistant messages to reduce token usage
    // (matches CLI behavior — long HTML responses are replaced with a placeholder)
    const messages: Anthropic.MessageParam[] = conv.messages.map((m, i) => {
        if (
            ...
            return { role: m.role as const, content: '[Previous dashboard response]' };
        }
    ...
    const tools: Anthropic.Tool[] = mcpTools.map((t, i) => ({
        ...
        // Cache the tool definitions — mark the last tool with cache_control
        ...(i === mcpTools.length - 1
 ...
 */
export async function* streamLookupResponse(
    conversationId: string,
): AsyncGenerator<StreamChunk> {
    if (backend === 'cli') {
        // CLI mode: fall back to full streamResponse (CLI handles its own prompt)
        yield* streamResponseCli(conversationId, model);
        return;
    }
